...
public class Pagination extends SimpleTagSupport {
			...
			for (int i = pgStart; i < pgEnd; i++) {
				if (i == currPage) {
					out.write("<li class=\"active\" " + (lastPage && i == totalPages ? " paginatorLast" : "") + "\">"
							+ "<span class=\"badge badge-primary\" > " + currPage + "</span>" + "</li>");
				} else {
	...
	private String constructLink(int page, String text, String className) {
		...
		link.append(">").append("<a href=\"").append(uri.replace("##",
				String.valueOf(page) + "&search=" + searchName + "&nombreElementPerPage=" + nombreElementPerPage))
